...
def preprocess_data(x_train, y_train, x_val, y_val, x_test, y_test, DATASET_FEATURE, num_classes, normalize=1):
    ...
    X_train = x_train.reshape(-1, DATASET_FEATURE).astype('float32')
    X_val   = x_val.reshape(-1, DATASET_FEATURE).astype('float32')
    X_test  = x_test.reshape(-1, DATASET_FEATURE).astype('float32')
...
def main():
    ...
    for ld in latent_dims:
        ...
        for run in range(3):
            ...
            mse = mean_squared_error(X_test, X_recon_1d)
            if DATASET == "CIFAR10":
                loss, acc = pretrained_model.evaluate(X_recon_img, Y_test, verbose=0)
            else:
    ...
    # Reshape for CNN evaluation 
    if DATASET == "CIFAR10":
        loss, acc = pretrained_model.evaluate(X_recon_final_img, Y_test, verbose=0)
    else:
        loss, acc = pretrained_model.evaluate(X_recon_final_1d, Y_test, verbose=0)
